...
export default function DashboardSalon() {
  ...
  const fetchDashboardData = async () => {
    try {
      setLoading(true);
      const [salonRes, employesRes, servicesRes, rendezVousRes] = await Promise.all([
        fetch(`/api/salon/${salonId}`), // Récupère un salon spécifique
        fetch(`/api/employe?salonId=${salonId}`),
        fetch(`/api/service?salonId=${salonId}`),
        fetch(`/api/rendezvous?salonId=${salonId}`),
      ]);

      const allResponses = { salonRes, employesRes, servicesRes, rendezVousRes };
      for (const [key, res] of Object.entries(allResponses)) {
        if (!res.ok) {
          const errorText = await res.text(); // Lire le contenu même s’il est HTML
          console.error(`Erreur API [${key}] - Status ${res.status}:`, errorText);
          toast.error(`Erreur lors du chargement des données [${key}]`);
          return; // Stoppe ici
        }
      }

      const [salonData, employesData, servicesData, rendezVousData] = await Promise.all([
        salonRes.json(),
        employesRes.json(),
        servicesRes.json(),
        rendezVousRes.json(),
      ...
      // Calcul des stats pour le salon spécifique
  ...
  return (
    <DashboardLayout title={`Dashboard ${salon?.nom || 'Salon'}`}>
      <div className="space-y-6">
        ...
        <div>
          <h1 className="text-3xl font-bold text-foreground">Dashboard {salon?.nom || 'Salon'}</h1>
          <p className="text-muted-foreground">Vue d'ensemble du salon {salon?.nom || ''}</p>
        </div>
